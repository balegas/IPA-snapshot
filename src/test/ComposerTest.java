...
public class ComposerTest {
	...
	public void test() {
		List<String> opNames = ImmutableList.of("opA", "opB", "opC",
				"opD" /* , "opE", "opF" */);
		Set<Operation> operations = opNames.stream().map(opN -> new DumbOperation(opN)).collect(Collectors.toSet());
		OperationComposer composer = new OperationComposer(operations, 3);
		List<List<Operation>> gen = composer.generate(new OperationPairTest("opA", "opB"), null);
